...
public class InventoryImpl implements Inventory
{
    ...
    public Item addItem(String code)
    {
        ...
        if(item.getItemDescriptor().isArtefact())
        {
            ...
            {
                net.afterday.compass.logging.Log.e("You can carry max " + MAX_ARTIFACTS_COUNT + " artifacts");
                return null;
            }
        ...
        if(item.hasModifier(Item.RADIATION_INSTANT))
        {
            this.hasRadInstant = true;
            o.addProperty("hasRadInstant", true);
        }
        if(item.hasModifier(Item.HEALTH_INSTANT))
        {
    ...
    {
        boolean isConsumed = activeBooster != null ? activeBooster.isConsumed() : false;
    ...
    private boolean removeItem(Item item)
    {
        ...
        {
            //inventoryItemsO.remove(item.toJson());  //TODO: remove from serialized
    ...
    private void validateInstants()
    {
        ...
        for (Item i : inventoryItems)
        {
            if(i.hasModifier(Item.HEALTH_INSTANT))
            {
                hInstant = true;
            }
            if(i.hasModifier(Item.RADIATION_INSTANT))
            {
                rInstant = true;
            }
        }
